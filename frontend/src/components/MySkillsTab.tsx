...
//   const handleUpdateFromAddCard = async (subSkillId: number) => {
...
//     try {
//       const url = new URL(`${BASE_URL}/skills/sub-skills/get/${subSkillId}`);
//       const res = await fetch(url.toString(), {
//         headers: { Authorization: `Bearer ${token}` },
//       });
...
//         throw new Error(errorData.detail || "Failed to fetch sub-skill details.");
//       }
...
//       setIsUpdateModalOpen(true);
//     } catch (err) {
//       console.error("Could not find the skill to update:", err);
//     }
//   };
...
//           onUpdateDuplicateSkill={handleUpdateFromAddCard}
//         />
//       ) : (
...
export default function MySkillsTab() {
  ...
  const [selectedSubSkill, setSelectedSubSkill] = useState<SubSkillUpdate | null>(null);
  const [isUpdateModalOpen, setIsUpdateModalOpen] = useState(false);
  const [isModalFromAddCard, setIsModalFromAddCard] = useState(false); // New state variable
  ...
  const handleUpdateClick = async (
    ...
    setIsModalFromAddCard(false); // Flag for regular update
    const token = localStorage.getItem("token");
    if (!token) return;
  ...
  const handleUpdateFromAddCard = async (subSkillId: number) => {
    setIsModalFromAddCard(true); // Flag for add card flow
    setShowAddSkill(false); // Hide the AddSkillCard temporarily
  ...
  // New function to handle modal closure and redirection
  const handleModalClose = () => {
      setSelectedSubSkill(null);
      setIsUpdateModalOpen(false);
      // If the modal was opened from the AddSkillCard, show it again
      if (isModalFromAddCard) {
          setShowAddSkill(true);
      }
      setIsModalFromAddCard(false); // Reset the flag
  };
        ...
        <UpdateSkillModal
          ...
          onUpdate={async (updatedSubSkill) => {
            try {
              ...
              setRefreshKey((prev) => prev + 1);
              handleModalClose(); // Close modal and redirect to AddSkillCard
            } catch (err) {
              console.error("❌ Update failed:", err);
            }
          }}
