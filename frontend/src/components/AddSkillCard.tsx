...
  const [selectedSubSkillToUpdate, setSelectedSubSkillToUpdate] = useState<SubSkillUpdate | null>(null);
...
        subskill_name: s.name,
        employee_proficiency: s.proficiency,
        experience: s.totalExperienceInMonths,
        certification_file_url: s.certificationFile instanceof File ? s.certificationFile.name : s.certificationFile || null,
        certification_creation_date: s.certificationCreationDate || null,
        certification_expiration_date: s.certificationExpirationDate || null,
      }))
...
  const handleOpenUpdateModal = async (subSkillId: number) => {
    try {
      const token = localStorage.getItem("token");
      const url = `${BACKEND_URL}/skills/sub-skills/get/${subSkillId}`;
      const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error("Failed to fetch sub-skill details.");
      const resData = await res.json();
      
      const mappedData: SubSkillUpdate = {
        emp_skill_id: resData.emp_skill_id,
        subskill_id: resData.subskill_id,
        skill_name: resData.skill_name,
        sub_skill_name: resData.sub_skill_name,
        proficiency_level: resData.proficiency_level,
        experience_years: resData.experience_years,
        has_certification: resData.has_certification,
        certification_file_url: resData.certification_file_url,
...
      };
      setSelectedSubSkillToUpdate(mappedData);
      setIsUpdateModalOpen(true);
    } catch (err) {
      console.error("Failed to fetch skill for update:", err);
      setPopupMessage({ text: "Failed to load skill data for update.", type: "error" });
      setTimeout(() => setPopupMessage(null), 4000);
    }
  };

  const handleUpdateSubmit = async (updatedData: SubSkillUpdate) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${BACKEND_URL}/skills/update/`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          emp_skill_id: updatedData.emp_skill_id,
          employee_id: updatedData.employee_id,
          subskill_id: updatedData.subskill_id,
          proficiency_level: updatedData.proficiency_level,
          experience_years: updatedData.experience_years,
          has_certification: updatedData.has_certification,
          certification_file_url: updatedData.certification_file_url ?? null,
          certification_creation_date: updatedData.certification_creation_date ?? null,
          certification_expiration_date: updatedData.certification_expiration_date ?? null,
        }),
      });
      if (!res.ok) throw new Error("Failed to update skill");

      // Remove the updated sub-skill from the list in the form
      setSubSkills(prev => prev.filter(s => s.id !== updatedData.subskill_id));

      // Clear the duplicate error for this skill
      setDuplicateErrors(prev => {
        const newState = { ...prev };
        delete newState[updatedData.subskill_id];
        return newState;
      });

      setPopupMessage({ text: "✅ Skill updated successfully!", type: "success" });
      setTimeout(() => setPopupMessage(null), 4000);
      
      setIsUpdateModalOpen(false);
      setSelectedSubSkillToUpdate(null);
    } catch (err: any) {
      setPopupMessage({ text: err.message || "Failed to update skill.", type: "error" });
      setTimeout(() => setPopupMessage(null), 4000);
    }
  };
  
...
                  <button
                    type="button"
                    onClick={() => handleOpenUpdateModal(subSkill.id as number)}
                    className="flex items-center gap-1 text-blue-500 text-xs hover:underline"
                  >
                    <Pencil className="w-4 h-4" />
                    Update Sub-skill
...
      </form>
      
      {isUpdateModalOpen && selectedSubSkillToUpdate && (
        <UpdateSkillModal
          subSkill={selectedSubSkillToUpdate}
