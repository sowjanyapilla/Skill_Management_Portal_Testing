...
//                     {activeTab === "approved" && (
//                       <>
//                         <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Updated Ratings</th>
//                         <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-96">Manager Comments</th>
//                       </>
//                     )}
...
//                     {/* Pending actions column: Approve | Edit | Reject */}
//                     {activeTab === "pending" && (
...
//                       <tr>
//                         <td className="px-6 py-4">{submission.employee.name}</td>
//                         <td className="px-6 py-4">{submission.skill_name}</td>
//                         <td className="px-6 py-4">{submission.sub_skill_name}</td>
//                         <td className="px-6 py-4">{renderStars(submission.employee_proficiency ?? 0)}</td>
//                         <td className="px-6 py-4">{submission.experience_years ?? "—"}</td>
//                         <td className="px-6 py-4">{submission.has_certification ? "Yes" : "No"}</td>
//                         <td className="px-6 py-4">{submission.created_at ? new Date(submission.created_at).toLocaleDateString() : "—"}</td>

//                         {activeTab !== "pending" && (
//                           <td className="px-6 py-4">{submission.updated_at ? new Date(submission.updated_at).toLocaleDateString() : "—"}</td>
//                         )}

//                         {activeTab === "approved" && (
//                           <>
//                             <td className="px-6 py-4">{submission.manager_proficiency != null ? renderStars(submission.manager_proficiency) : "—"}</td>
//                             <td className="px-6 py-4 max-w-[36rem] break-words">{submission.manager_comments ?? "—"}</td>
//                           </>
//                         )}
...
//                               }}
//                               className="px-2 py-1 text-xs font-medium text-indigo-700 bg-indigo-100 rounded-lg hover:bg-indigo-200"
//                             >
//                               Edit
//                             </button>
...
//                                 toggleRowExpansion(submission.id);
//                               }}
//                               className="px-2 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-lg hover:bg-red-200"
//                             >
//                               Reject
//                             </button>
//                           </td>
...
//                       {expandedRows.has(submission.id) && (
//                         <tr>
//                           <td colSpan={activeTab === "pending" ? 8 : activeTab === "approved" ? 10 : 9} className="px-6 py-4 bg-gray-50">
              ...
              <table className="min-w-full table-fixed divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentApprovals.map((submission) => (
                    <React.Fragment key={submission.id}>
                      <tr>
                        <td className="px-6 py-4 max-w-[12rem]"><div className="truncate" title={submission.employee.name}>{submission.employee.name}</div></td>
                        <td className="px-6 py-4 max-w-[12rem]" ><div className="truncate" title={submission.skill_name}>{submission.skill_name}</div></td>
                        <td className="px-6 py-4 max-w-[12rem]"><div className="truncate" title={submission.sub_skill_name}>{submission.sub_skill_name}</div></td>
                        <td className="px-6 py-4">{renderStars(submission.employee_proficiency ?? 0)}</td>
                        <td className="px-6 py-4">{submission.experience_years ?? "—"}</td>
                        <td className="px-6 py-4">{submission.has_certification ? "Yes" : "No"}</td>
                        <td className="px-6 py-4"><div className="truncate" title={submission.created_at ? new Date(submission.created_at).toLocaleDateString() : '—'}>{submission.created_at ? new Date(submission.created_at).toLocaleDateString() : '—'}</div></td>

                        {activeTab !== "pending" && (
                          <td className="px-6 py-4"><div className="truncate" title={submission.updated_at ? new Date(submission.updated_at).toLocaleDateString() : '—'}>{submission.updated_at ? new Date(submission.updated_at).toLocaleDateString() : '—'}</div></td>
                        )}

                        {activeTab === "approved" && (
                          <>
                            <td className="px-6 py-4">{submission.manager_proficiency != null ? renderStars(submission.manager_proficiency) : "—"}</td>
                            <td className="px-6 py-4 max-w-[36rem]"><div className="truncate" title={submission.manager_comments ?? '—'}>{submission.manager_comments ?? '—'}</div></td>
                          </>
                        )}
